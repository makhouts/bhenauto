...
export const metadata = { title: `Admin Dashboard | ${DEFAULT_TENANT_BOOTSTRAP.adminDisplayName}` };
...
export default async function AdminDashboardPage() {
    ...
    const [
        ...
        tenant.features.contacts ? prisma.contact.count({ where: { read: false } }) : Promise.resolve(0),
        tenant.features.contacts ? prisma.contact.count() : Promise.resolve(0),
        tenant.features.appointments ? prisma.appointment.count({ where: { status: "pending" } }) : Promise.resolve(0),
        tenant.features.appointments ? prisma.appointment.count({ where: { status: "confirmed" } }) : Promise.resolve(0),
    ...
    return (
        <AdminPage className="max-w-[1400px] space-y-8">
            ...
            <div className={`grid grid-cols-1 gap-6 ${tenant.features.contacts ? "xl:grid-cols-2" : ""}`}>
