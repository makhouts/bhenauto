...
import { isCurrentTenantFeatureEnabled } from "@/lib/tenant.server";
...
function isAllowedCarImage(value: string): boolean {
    ...
    try {
        ...
        const allowedHosts = new Set(
            getTenantBootstrapImageHosts().map((origin) => new URL(origin).hostname),
        );
...
export async function retryCarAutoscoutSync(id: string) {
    await requireAdmin();
    if (!await isCurrentTenantFeatureEnabled("autoscout24")) {
        return { error: "AutoScout24 sync is not enabled for this tenant." };
    }
    try {
...
export async function updateCarStatus(id: string, status: "beschikbaar" | "gereserveerd" | "verkocht") {
    await requireAdmin();
    const autoscoutEnabled = await isCurrentTenantFeatureEnabled("autoscout24");
    try {
        await prisma.car.update({
            where: { id },
            data: {
                ...
                autoscoutSyncStatus: autoscoutEnabled
                    ? (status === "verkocht" ? "pending-delete" : "pending")
                    : "skipped",
                autoscoutSyncError: null,
            },
        });
        ...
        revalidateLocalizedPath("/inventory");
        revalidateLocalizedPath("");
        return { success: true, autoscoutQueued: autoscoutEnabled };
    } catch (error) {
...
export async function deleteCar(id: string) {
    ...
    try {
        ...
        await cancelPendingAutoScoutSyncJobsForCar(
            id,
            "Vehicle deleted from tenant inventory.",
        );
...
const CarInputSchema = z.object({
    ...
    images: z.array(z.string().min(1).refine(isAllowedCarImage, "Image must be an approved R2 key or CDN URL")).min(0).max(50),
});
...
export async function saveCar(data: unknown) {
    ...
    try {
        ...
        const shouldSyncWithAutoscout = autoscoutEnabled && syncWithAutoscout;
        const autoscoutValidationIssues = shouldSyncWithAutoscout
            ? validateAutoScoutListingValues({
                ...carData,
            ...
            await prisma.$transaction([
                prisma.car.update({
                    ...
                    data: {
                        ...
                        autoscoutSyncStatus: shouldSyncWithAutoscout ? pendingSyncStatus : "skipped",
                        autoscoutSyncError: null,
                    },
                }),
        ...
        if (savedCarId && shouldSyncWithAutoscout) {
            await enqueueAutoScoutSyncJob({
                carId: savedCarId,
                action: syncAction,
                resetAttempts: true,
        ...
        return { success: true, autoscoutQueued: Boolean(savedCarId && shouldSyncWithAutoscout) };
    } catch (error) {
