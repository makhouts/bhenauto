...
import { PublicEmailLink } from "@/components/PublicEmail";
...
export default function Footer({ dict, workshopEnabled }: FooterProps) {
        ...
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 box-border mt-2">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-6 mb-12 lg:mb-16">
                    ...
                    <div className="space-y-4 lg:col-span-5 pr-4">
                        <Link href={`/${locale}`} className="inline-block">
                            <Image
                                src={logo}
                                alt={`${DEFAULT_TENANT_BOOTSTRAP.displayName} logo`}
                                width={170}
                            ...
                            <ul className="space-y-3 text-sm">
                                <li className="flex items-start gap-2 group">
                                    ...
                                    <span className="text-white group-hover:text-white/80 transition-colors duration-300">
                                        {DEFAULT_TENANT_BOOTSTRAP.addressLine}, {DEFAULT_TENANT_BOOTSTRAP.postalCode} {DEFAULT_TENANT_BOOTSTRAP.city}
                                    </span>
                                </li>
                                <li className="flex items-center gap-2 group">
                                    <Phone className="w-4 h-4 text-[#d91c1c] shrink-0 group-hover:scale-110 transition-all duration-300" />
                                    <a href={`tel:${DEFAULT_TENANT_BOOTSTRAP.phone.replace(/\s+/g, "")}`} className="text-white group-hover:text-white/80 transition-colors duration-300">{DEFAULT_TENANT_BOOTSTRAP.phone}</a>
                                </li>
                                <li className="flex items-center gap-2 group">
                                    <Mail className="w-4 h-4 text-[#d91c1c] shrink-0 group-hover:scale-110 transition-all duration-300" />
                                    <PublicEmailLink className="text-white group-hover:text-white/80 transition-colors duration-300" />
                                </li>
                            </ul>
                        </div>
                ...
                <div className="flex flex-col sm:flex-row justify-end items-center gap-4 pt-6 relative z-10 w-full text-xs text-white/50 border-t border-white/10">
                    <p>&copy; {new Date().getFullYear()} {DEFAULT_TENANT_BOOTSTRAP.displayName}. {dict.copyright}</p>
                </div>
                ...
                <div className="absolute bottom-[-10%] left-4 sm:left-6 lg:left-8 pointer-events-none select-none z-0">
                    <span className="text-[14vw] md:text-[8rem] lg:text-[10rem] font-headings font-black tracking-tighter text-white/[0.04] uppercase leading-none">
                        {DEFAULT_TENANT_BOOTSTRAP.displayName.toUpperCase()}
                    </span>
                </div>
            </div>
